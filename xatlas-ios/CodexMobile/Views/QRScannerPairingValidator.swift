...
private let qrScannerBridgeUpdateCommand = "npm install -g xatlas@latest"
...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    ...
    if looksLikexatlasPairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older xatlas bridge. Update the npm package on your Mac to the latest release before scanning a new QR code."
            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest xatlas bridge.")
}

private func looksLikexatlasPairingPayload(_ data: Data) -> Bool {
