...
final class CodexServiceTierTests: XCTestCase {
    ...
    func testUnsupportedServiceTierDisablesFutureRetriesAndShowsUpdatePromptOnce() async throws {
        ...
        XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update xatlas on your Mac to use Speed controls")
        XCTAssertEqual(
            service.bridgeUpdatePrompt?.message,
            "This Mac bridge does not support the selected speed setting yet. Update the xatlas npm package to use Fast Mode and other speed controls."
        )
        XCTAssertEqual(service.bridgeUpdatePrompt?.command, "npm install -g xatlas@1.1.4")
    }
