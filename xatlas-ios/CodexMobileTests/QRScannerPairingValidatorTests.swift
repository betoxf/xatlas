...
final class QRScannerPairingValidatorTests: XCTestCase {
    func testVersionMismatchRequiresBridgeUpdateBeforeScanning() {
        ...
        XCTAssertEqual(prompt.title, "Update xatlas on your Mac before scanning")
        XCTAssertEqual(prompt.command, "npm install -g xatlas@latest")
        XCTAssertTrue(prompt.message.contains("different xatlas npm version"))
    }
    ...
    func testLegacyBridgePayloadRequiresBridgeUpdateBeforeScanning() {
        ...
        XCTAssertEqual(prompt.command, "npm install -g xatlas@latest")
        XCTAssertTrue(prompt.message.contains("older xatlas bridge"))
    }
