...
final class CodexThreadForkTests: XCTestCase {
    ...
    func testLocalForkUsesSourceThreadWorkingDirectory() async throws {
        ...
        XCTAssertEqual(capturedForkParams["cwd"]?.stringValue, "/tmp/xatlas")
        ...
        XCTAssertEqual(service.thread(for: "fork-local")?.gitWorkingDirectory, "/tmp/xatlas")
    }
    ...
    func testWorktreeForkUsesProvidedProjectPath() async throws {
        ...
        service.requestTransportOverride = { method, params in
            ...
            case "thread/fork":
                ...
                return RPCMessage(
                    id: .string(UUID().uuidString),
                    result: .object([
                        "cwd": .string("/tmp/xatlas-worktree"),
                        "thread": .object([
        ...
        let forkedThread = try await service.forkThreadIfReady(
            from: "source-thread",
            target: .projectPath("/tmp/xatlas-worktree")
        )

        XCTAssertEqual(capturedForkParams["cwd"]?.stringValue, "/tmp/xatlas-worktree")
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/xatlas-worktree")
    }
    ...
    func testForkStillReturnsCreatedThreadWhenHydrationFails() async throws {
        ...
        XCTAssertEqual(service.thread(for: "fork-partial")?.gitWorkingDirectory, "/tmp/xatlas-worktree")
    }
    ...
    func testForkFallsBackToMinimalRequestWhenOverridesAreRejected() async throws {
        ...
        XCTAssertEqual(forkRequests.first?["cwd"]?.stringValue, "/tmp/xatlas-worktree")
        ...
        XCTAssertEqual(resumeRequests.first?["cwd"]?.stringValue, "/tmp/xatlas-worktree")
        XCTAssertEqual(resumeRequests.first?["model"]?.stringValue, "gpt-5.4")
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/xatlas-worktree")
        XCTAssertEqual(service.thread(for: "fork-minimal")?.gitWorkingDirectory, "/tmp/xatlas-worktree")
    }
    ...
    func testUnsupportedThreadForkDisablesCapabilityAndShowsUpdatePrompt() async {
        ...
        do {
            ...
            XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update xatlas on your Mac to use /fork")
            XCTAssertEqual(
                service.bridgeUpdatePrompt?.message,
                "This Mac bridge does not support native conversation forks yet. Update the xatlas npm package to use /fork and worktree fork flows."
            )
            XCTAssertEqual(service.bridgeUpdatePrompt?.command, "npm install -g xatlas@latest")
        }
    }
    ...
    func testLocalForkFallsBackToCurrentWorktreeWhenLocalCheckoutIsUnavailable() {
        ...
        let worktreePath = tempRoot
            .appendingPathComponent(".codex/worktrees/a8b4/xatlas-website", isDirectory: true)
    ...
    func testLocalForkAcceptsRemotePathsWithoutCheckingClientFilesystem() {
        let worktreeThread = CodexThread(
            id: "source-thread",
            title: "Source",
            cwd: "/Users/xatlas/.codex/worktrees/a8b4/xatlas-website",
            model: "gpt-5.4",
            modelProvider: "openai"
        )
        ...
        let localForkPath = TurnThreadForkCoordinator.localForkProjectPath(
            for: worktreeThread,
            localCheckoutPath: "/Users/xatlas/Developer/xatlas/xatlas-website"
        )

        XCTAssertEqual(localForkPath, "/Users/xatlas/Developer/xatlas/xatlas-website")
    }
