...
final class SidebarThreadGroupingTests: XCTestCase {
    ...
    func testMakeGroupsMarksCodexManagedWorktreesInLabelAndIcon() {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let threads = [
            makeThread(id: "main-thread", updatedAt: now, cwd: "/Users/me/work/xatlas"),
            makeThread(
                id: "worktree-thread",
                updatedAt: now.addingTimeInterval(-60),
                cwd: "/Users/me/.codex/worktrees/ce15/xatlas"
            ),
        ]

        let groups = SidebarThreadGrouping.makeGroups(from: threads, now: now)
        let mainGroup = try XCTUnwrap(groups.first(where: { $0.projectPath == "/Users/me/work/xatlas" }))
        let worktreeGroup = try XCTUnwrap(
            groups.first(where: { $0.projectPath == "/Users/me/.codex/worktrees/ce15/xatlas" })
        )

        XCTAssertEqual(mainGroup.label, "xatlas")
        XCTAssertEqual(mainGroup.iconSystemName, "folder")
        XCTAssertEqual(worktreeGroup.label, "xatlas 15")
    ...
    func testMakeProjectChoicesKeepWorktreeSelectionCompactWithoutShowingPathInLabel() {
        ...
        XCTAssertEqual(labelsByPath["/Users/me/work/xatlas"]?.label, "xatlas")
        XCTAssertEqual(labelsByPath["/Users/me/work/xatlas"]?.iconSystemName, "folder")
        XCTAssertEqual(labelsByPath["/Users/me/.codex/worktrees/ce15/xatlas"]?.label, "xatlas 15")
        XCTAssertEqual(labelsByPath["/Users/me/.codex/worktrees/ce15/xatlas"]?.iconSystemName, "arrow.triangle.branch")
    }
