...
async function resolveVoiceAuth(sendCodexRequest) {
  ...
  try {
    ...
    console.error(`[xatlas-bridge] voice/resolveAuth: getAuthStatus RPC failed: ${err.message}`);
  ...
  if (!token) {
    console.error(`[xatlas-bridge] voice/resolveAuth: no token. authMethod=${authMethod || "none"} requiresOpenaiAuth=${authStatus?.requiresOpenaiAuth}`);
