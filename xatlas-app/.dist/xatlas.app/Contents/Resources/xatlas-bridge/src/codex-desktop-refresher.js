...
// Purpose: Debounced Mac desktop refresh controller for xatlas.app after phone-authored conversation changes.
// Layer: CLI helper
...
const DEFAULT_BUNDLE_ID = "com.xatlas.app";
const DEFAULT_APP_PATH = "/Applications/xatlas.app";
...
const NEW_THREAD_DEEP_LINK = "xatlas://threads/new";
...
class CodexDesktopRefresher {
  ...
  // Keeps one lightweight rollout watcher alive for the current Xatlas-controlled thread.
...
function readBridgeConfig({
  ...
  const codexEndpoint = readFirstDefinedEnv(
    ["XATLAS_CODEX_ENDPOINT", "PHODEX_CODEX_ENDPOINT"],
    "",
    env
  );
  const refreshCommand = readFirstDefinedEnv(
    ["XATLAS_REFRESH_COMMAND", "PHODEX_ON_PHONE_MESSAGE"],
    "",
    env
  );
  const explicitRefreshEnabled = readOptionalBooleanEnv(["XATLAS_REFRESH_ENABLED"], env);
  // Desktop refresh is opt-in for now because xatlas.app still lacks true live updates.
  const defaultRefreshEnabled = false;
  return {
    relayUrl,
    pushServiceUrl: readFirstDefinedEnv(
      ["XATLAS_PUSH_SERVICE_URL"],
      defaultPushServiceUrl,
      env
    ),
    pushPreviewMaxChars: parseIntegerEnv(
      readFirstDefinedEnv(["XATLAS_PUSH_PREVIEW_MAX_CHARS"], "160", env),
      160
    ),
    ...
    refreshDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(["XATLAS_REFRESH_DEBOUNCE_MS"], String(DEFAULT_DEBOUNCE_MS), env),
      DEFAULT_DEBOUNCE_MS
    ),
    codexEndpoint,
    refreshCommand,
    codexBundleId: readFirstDefinedEnv(["XATLAS_CODEX_BUNDLE_ID"], DEFAULT_BUNDLE_ID, env),
...
function isSourceCheckout(runtimeRoot, fsImpl) {
  ...
  return path.basename(runtimeRoot) === "xatlas-bridge"
...
function buildThreadDeepLink(threadId) {
  return `xatlas://threads/${threadId}`;
}
