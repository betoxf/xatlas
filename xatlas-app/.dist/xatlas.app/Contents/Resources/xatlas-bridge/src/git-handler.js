...
async function stashChangesForWorktreeHandoff(cwd) {
  const stashLabel = `xatlas-worktree-handoff-${randomBytes(6).toString("hex")}`;
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(os.tmpdir(), `xatlas-worktree-copy-${randomBytes(6).toString("hex")}.patch`);
...
function normalizeCreatedBranchName(rawName) {
  ...
  if (normalized.startsWith(	"xatlas/")) {
    return normalized;
  }
  return `xatlas/${normalized}`;
}
