...
const DEFAULT_STORE_DIR = path.join(os.homedir(), ".xatlas");
const DEFAULT_STORE_FILE = path.join(DEFAULT_STORE_DIR, "device-state.json");
const KEYCHAIN_SERVICE = "com.xatlas.bridge.device-state";
...
function rememberTrustedPhone(state, phoneDeviceId, phoneIdentityPublicKey, { persist = true } = {}) {
  ...
  // Xatlas supports one trusted iPhone per Mac, so a new trust record replaces old ones.
...
function resolveStoreDir() {
  return normalizeNonEmptyString(process.env.XATLAS_DEVICE_STATE_DIR) || DEFAULT_STORE_DIR;
}

function resolveStoreFile() {
  return normalizeNonEmptyString(process.env.XATLAS_DEVICE_STATE_FILE)
    || path.join(resolveStoreDir(), "device-state.json");
}

function resolveKeychainMirrorFile() {
  return normalizeNonEmptyString(process.env.XATLAS_DEVICE_STATE_KEYCHAIN_MOCK_FILE);
}
