...
test("loadOrCreateBridgeDeviceState throws when only the legacy Keychain mirror is corrupted", () => {
  withTempDeviceStateEnv(({ keychainMirrorFile, canonicalStateFile }) => {
    ...
    assert.throws(
      () => loadOrCreateBridgeDeviceState(),
      /saved Xatlas pairing state in legacy Keychain bridge state is unreadable/i
    );
...
test("loadOrCreateBridgeDeviceState throws when the canonical file is corrupted and no fallback exists", () => {
  withTempDeviceStateEnv(({ canonicalStateFile }) => {
    ...
    assert.throws(
      () => loadOrCreateBridgeDeviceState(),
      /saved Xatlas pairing state in device-state\.json is unreadable/i
    );
  });
});
...
function withTempDeviceStateEnv(run) {
  ...
  const previousMirror = process.env.XATLAS_DEVICE_STATE_KEYCHAIN_MOCK_FILE;
  ...
  process.env.XATLAS_DEVICE_STATE_DIR = tempRoot;
  process.env.XATLAS_DEVICE_STATE_KEYCHAIN_MOCK_FILE = keychainMirrorFile;
  ...
  try {
    ...
    if (previousMirror === undefined) {
      delete process.env.XATLAS_DEVICE_STATE_KEYCHAIN_MOCK_FILE;
    } else {
      process.env.XATLAS_DEVICE_STATE_KEYCHAIN_MOCK_FILE = previousMirror;
    }
...
function readCanonicalStateFromDisk() {
  const canonicalStateFile = path.join(process.env.XATLAS_DEVICE_STATE_DIR, "device-state.json");
