...
// Purpose: Handles explicit desktop-handoff bridge actions for xatlas.app.
...
async function continueOnMac(
  ...
  const targetUrl = `xatlas://threads/${threadId}`;
  ...
  // If xatlas.app is already open, explicit handoff should still feel like a
  ...
  if (desktopKnown && !appRunning) {
    try {
      ...
      throw desktopError(
        "handoff_failed",
        "Could not open xatlas.app on this Mac.",
        error
      );
    }
  ...
  try {
    ...
    throw desktopError(
      "handoff_failed",
      "Could not force close and reopen xatlas.app on this Mac.",
      error
    );
  }
...
async function waitForAppExit(appPath, executor, isAppRunning) {
  ...
  throw desktopError("handoff_timeout", "Timed out waiting for xatlas.app to close.");
}
