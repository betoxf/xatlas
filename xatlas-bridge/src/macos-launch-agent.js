...
const SERVICE_LABEL = "com.xatlas.bridge";
...
async function startMacOSBridgeService({
  ...
  cliPath = path.resolve(__dirname, "..", "bin", "xatlas-bridge.js"),
  ...
  ensureXatlasStateDir({ env, fsImpl, osImpl });
  ensureXatlasLogsDir({ env, fsImpl, osImpl });
...
function writeLaunchAgentPlist({
  ...
  const stateDir = resolveXatlasStateDir({ env, osImpl });
...
<dict>
  ...
  <dict>
    ...
    <key>XATLAS_DEVICE_STATE_DIR</key>
...
function assertRelayConfigured(config) {
  ...
  throw new Error("No relay URL configured. Run ./run-local-xatlas.sh or set XATLAS_RELAY before enabling the macOS bridge service.");
}
