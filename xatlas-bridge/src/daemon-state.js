...
const DEFAULT_STATE_DIR_NAME = ".xatlas";
...
// Reuses the existing Xatlas state root so daemon mode keeps the same local-first storage model.
function resolveXatlasStateDir({ env = process.env, osImpl = os } = {}) {
  return normalizeNonEmptyString(env.XATLAS_DEVICE_STATE_DIR)
    || path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME);
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolveXatlasStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolveXatlasStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolveXatlasStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolveXatlasStateDir(options), LOGS_DIR);
}
...
function ensureXatlasStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolveXatlasStateDir(options), { recursive: true });
}

function ensureXatlasLogsDir({ fsImpl = fs, ...options } = {}) {
