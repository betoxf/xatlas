...
async function resolveVoiceAuth(sendCodexRequest) {
  ...
  try {
    ...
    console.error(`[xatlas] voice/resolveAuth: getAuthStatus RPC failed: ${err.message}`);
  ...
  if (!token) {
    console.error(`[xatlas] voice/resolveAuth: no token. authMethod=${authMethod || "none"} requiresOpenaiAuth=${authStatus?.requiresOpenaiAuth}`);
