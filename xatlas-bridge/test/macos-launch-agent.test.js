...
test("buildLaunchAgentPlist points launchd at run-service with remodex state paths", () => {
  const plist = buildLaunchAgentPlist({
    ...
    stateDir: "/Users/tester/.xatlas",
    stdoutLogPath: "/Users/tester/.xatlas/logs/bridge.stdout.log",
    stderrLogPath: "/Users/tester/.xatlas/logs/bridge.stderr.log",
    nodePath: "/usr/local/bin/node",
    cliPath: "/tmp/remodex/bin/xatlas-bridge.js",
  });

  assert.match(plist, /<string>com\.xatlas\.bridge<\/string>/);
  ...
  assert.match(plist, /<key>XATLAS_DEVICE_STATE_DIR<\/key>/);
});
...
test("resolveLaunchAgentPlistPath writes into the user's LaunchAgents folder", () => {
  assert.equal(
    ...
    path.join("/Users/tester", "Library", "LaunchAgents", "com.xatlas.bridge.plist")
  );
});
    ...
    assert.deepEqual(calls, [
      [
        ...
        [
          "bootout",
          `gui/${process.getuid()}`,
          path.join(process.env.HOME, "Library", "LaunchAgents", "com.xatlas.bridge.plist"),
        ],
      ],
      [
        "launchctl",
        [
          "bootout",
          `gui/${process.getuid()}/com.xatlas.bridge`,
        ],
      ],
    ]);
  });
...
test("getMacOSBridgeServiceStatus reports launchd + runtime metadata together", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const plistPath = path.join(rootDir, "LaunchAgents", "com.xatlas.bridge.plist");
    fs.mkdirSync(path.dirname(plistPath), { recursive: true });
    fs.writeFileSync(plistPath, "plist");

    const status = getMacOSBridgeServiceStatus({
      platform: "darwin",
      env: { HOME: rootDir, XATLAS_DEVICE_STATE_DIR: rootDir },
      execFileSyncImpl() {
        return "pid = 55";
      },
    });
...
function withTempDaemonEnv(run) {
  const previousDir = process.env.XATLAS_DEVICE_STATE_DIR;
  const previousHome = process.env.HOME;
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "remodex-launch-agent-"));
  process.env.XATLAS_DEVICE_STATE_DIR = rootDir;
  ...
  try {
    ...
    if (previousDir === undefined) {
      delete process.env.XATLAS_DEVICE_STATE_DIR;
    } else {
      process.env.XATLAS_DEVICE_STATE_DIR = previousDir;
    }
