...
- **stdio bridge**: `xatlas-app/scripts/mcp-stdio-bridge.js` adapts stdio MCP transport (used by Codex) to the Swift app's HTTP server. Reads active port from the persisted state file.
...
- **`services/`**: `tmuxManager.ts` (session prefix `xvsc_`), `tmuxStreamBridge.ts`, `terminalWatcher.ts`, `agentDiscovery.ts` (detects Codex/Zai/Codex/Aider), `openRouterService.ts` (AI terminal naming)
...
- The `.mcp.json` at repo root configures Codex's MCP connections (filesystem server + xatlas native app).
- `vscode-extension/AGENTS.md` is an AI agent prompt/tool reference, not a developer guide.
